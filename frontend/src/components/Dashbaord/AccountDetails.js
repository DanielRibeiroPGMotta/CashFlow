...
export default function AccountDetails() {
  ...
  const dailyIncome = account?.transactions?.reduce((acc, transaction) => {
    const event = new Date(transaction?.date)
    const date = event.getDate();
    if (date === day && transaction?.transactionType === "Income") {
      return acc + transaction?.amount;
    } else {
      return acc;
    }
  ...
  const dailyExpenses = account?.transactions?.reduce((acc, transaction) => {
    const event = new Date(transaction?.date)
    const date = event.getDate();
    if (date === day && transaction?.transactionType === "Expenses") {
      return acc + transaction?.amount;
    } else {
      return acc;
    }
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <div className="mx-auto max-w-4xl text-center">
                ...
                <p className="mt-3 text-xl text-gray-500 sm:mt-4">
                  Não deixe suas despesas ultrapassar suas receitas
                </p>
              </div>
            </div>
                    ...
                    <dl className="rounded-lg bg-white shadow-lg sm:grid sm:grid-cols-3">
                      <div className="flex flex-col border-b border-gray-100 p-6 text-center sm:border-0 sm:border-r">
                        ...
                        <dd className=" text-5xl font-bold tracking-tight text-indigo-600">
                          ${" "}
                          {totalIncome +
                            totalExpenses + account?.initialBalance
                            }
                        </dd>
                      </div>
